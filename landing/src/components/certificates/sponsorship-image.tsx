...
export const SponsorshipImage = ({ receiver, id }: { receiver: string, id: number; }) => {
    const colorIndex = BigNumber.from(receiver)
        .add(BigNumber.from(id))
        .mod(colors.length)
        .toNumber();
    const c = colors[colorIndex];

    const strokeWidthIndex = BigNumber.from(receiver)
        .sub(BigNumber.from(id))
        .mod(100)
        .toNumber();
